...
public sealed class ReviewPackEvidenceTests
{
    ...
    public void CompletePackageEvidenceIsPassedWithExactManifest()
    {
        ...
        Assert.Equal(EvidenceStatus.Passed, result.Status);
        Assert.Equal(1, result.PackageFileCount);
    }
    ...
    public void BuildLogWithFailureIsRejected()
    {
        ...
        Assert.Equal(EvidenceStatus.Failed, BuildEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework).Debug.Status);
    }
    ...
    public void MissingReleaseLogIsRejected()
    {
        ...
        Assert.Equal(EvidenceStatus.Failed, BuildEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework).Release.Status);
    }
    ...
    public void AssemblyMetadataMismatchIsRejected()
    {
        ...
        Assert.Equal(EvidenceStatus.Failed, BuildEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework).AssemblyInventory.Status);
    }

    [Fact]
    public void CliDoctorSuccessFalseIsRejected()
    {
        using TemporaryDirectory fixture = new(); CreateBuildAndCliEvidence(fixture); fixture.Write("cli/doctor.json", DoctorJson(false).Replace("\"targetFramework\"", "\"semanticVersion\": \"0.2.0-dev\", \"targetFramework\"", StringComparison.Ordinal));
        Assert.Equal(EvidenceStatus.Failed, CliEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework).Doctor.Status);
    }
    ...
    public void Phase01VectorMismatchIsRejected()
    {
        ...
        Assert.Equal(EvidenceStatus.Failed, CliEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework).Phase01SelfTest.Status);
    }
    ...
    public void Phase02CanonicalDigestMismatchIsRejected()
    {
        ...
        Assert.Equal(EvidenceStatus.Failed, CliEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework).Phase02DomainSelfTest.Status);
    }

    [Fact]
    public void CliVersionMismatchIsRejected()
    {
        using TemporaryDirectory fixture = new(); CreateBuildAndCliEvidence(fixture); string path = fixture.PathOf("cli/version.txt"); fixture.Write("cli/version.txt", File.ReadAllText(path).Replace("0.2.0-dev", "0.1.0-dev", StringComparison.Ordinal));
        Assert.Equal(EvidenceStatus.Failed, CliEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework).Version.Status);
    }
    ...
    public void ValidBuildAndCliFixturesPass()
    {
        ...
        Assert.True(BuildEvidenceValidator.IsPassed(BuildEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework)));
        Assert.True(CliEvidenceValidator.IsPassed(CliEvidenceValidator.Evaluate(fixture.Root, Commit, "0.2.0-dev", Framework)));
    }
    ...
    private static void CreateBuildAndCliEvidence(TemporaryDirectory fixture)
    {
        ...
        AssemblyInventoryEntry[] inventory = assemblies.Select(name => new AssemblyInventoryEntry(name, "Release", $"src/{name}/bin/Release/net10.0/{name}.dll", "0.2.0.0", $"0.2.0-dev+{Commit}", Commit, Framework)).ToArray();
    ...
    private static string DoctorJson(bool success) => $$"""
        {
          ...
          "build": { "semanticVersion": "0.2.0-dev", "gitCommit": "{{Commit}}", "targetFramework": "{{Framework}}" },
          "checks": [
