...
internal sealed class RngDomainIdJsonConverter : JsonConverter<RngDomainId> { public override RngDomainId Read(ref Utf8JsonReader r, Type t, JsonSerializerOptions o) => r.TokenType == JsonTokenType.String ? RngDomainId.Parse(r.GetString()!) : throw new JsonException(); public override void Write(Utf8JsonWriter w, RngDomainId v, JsonSerializerOptions o) { if (v.IsEmpty) throw new JsonException("Default RNG domains cannot be written."); w.WriteStringValue(v.ToString()); } }
internal sealed class RngScopeKeyJsonConverter : JsonConverter<RngScopeKey> { public override RngScopeKey Read(ref Utf8JsonReader r, Type t, JsonSerializerOptions o) => r.TokenType == JsonTokenType.String ? RngScopeKey.Parse(r.GetString()!) : throw new JsonException(); public override void Write(Utf8JsonWriter w, RngScopeKey v, JsonSerializerOptions o) { if (v.IsEmpty) throw new JsonException("Default RNG scopes cannot be written."); w.WriteStringValue(v.ToString()); } }
...
internal sealed class RngSampleAddressJsonConverter : JsonConverter<RngSampleAddress>
{
    ...
    public override void Write(Utf8JsonWriter w, RngSampleAddress v, JsonSerializerOptions o) { if (v.Domain.IsEmpty || v.Scope.IsEmpty) throw new JsonException("Default RNG sample addresses cannot be written."); w.WriteStartObject(); w.WriteString("domain", v.Domain.ToString()); w.WriteString("scope", v.Scope.ToString()); w.WriteString("sampleIndex", v.SampleIndex.ToString(CultureInfo.InvariantCulture)); w.WriteEndObject(); }
}
